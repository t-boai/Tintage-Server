...
export const productsFeatured = async (
  ...
  try {
    const cachedData = await redisClient.get(HOME_CACHE_KEYS.FEATURED);
    const limit = 6;
    let productsFinal;

    if (cachedData) {
      productsFinal = JSON.parse(cachedData);
    } else {
      productsFinal = await runWithSingleflight(
        HOME_CACHE_KEYS.FEATURED,
        async () => {
          ...
          const formatted = products.map((item) => {
            const actuallyNew = isActuallyNew(
              item.isNewProduct,
              item.createdAt,
            );
...
export const dailyDiscover = async (
  ...
  try {
    ...
    const second = Math.max(1, Math.floor((endOfDay.getTime() - now) / 1000));

    let productsFinal;
    const cachedData = await redisClient.get(HOME_CACHE_KEYS.DAILY_DISCOVER);

    if (cachedData) {
      productsFinal = JSON.parse(cachedData);
    } else {
      productsFinal = await runWithSingleflight(
        HOME_CACHE_KEYS.DAILY_DISCOVER,
        async () => {
    ...
    res.setHeader(
      "Cache-Control",
      `public, max-age=0, s-maxage=${second}, stale-while-revalidate=60`,
    );
